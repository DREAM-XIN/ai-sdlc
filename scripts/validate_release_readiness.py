...
def main():
    ...
    require(
        release.get("release_policy", {}).get("publish_tag_after_blockers_clear") is True,
        "release policy must keep tag publication behind declared blockers",
        errors,
    )
    require(
        release.get("release_policy", {}).get("production_caller_pin") == "full-commit-sha",
        "release policy must require full-commit-sha production caller pins",
        errors,
    )
    ...
    for critical in (
        ...
        require(critical in validators, f"critical security validator omitted from release baseline: {critical}", errors)
    ...
    print(
        ...
        f"{len(validators)} CI validators"
    )
