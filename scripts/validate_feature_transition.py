...
def main():
    ...
    completed = apply_event(
        started["manifest"],
        event("F-0030", [{"kind": "stage", "id": "design", "status": "DONE"}], "2026-08-07T11:12:00Z"),
    )
    ...
    blocked = apply_event(initial, event("F-0030", [{"kind": "stage", "id": "design", "status": "BLOCKED", "reason": "missing API contract"}]))
    ...
    rework = apply_event(
        blocked["manifest"],
        event("F-0030", [{"kind": "stage", "id": "design", "status": "READY"}], "2026-08-07T11:13:00Z"),
    )
    ...
    duplicate_evidence = apply_event(
        ...
        event("F-0030", [{"kind": "evidence", "record": {"id": "EVID-DESIGN", "type": "review", "status": "pass", "uri": "review://again"}}]),
    )
    require(duplicate_evidence["outcome"] == "INVALID", "duplicate evidence unexpectedly passed")

    mismatch = apply_event(initial, event("F-OTHER", [{"kind": "stage", "id": "design", "status": "WORKING"}]))
    ...
    illegal = apply_event(done_manifest, event("F-0030", [{"kind": "stage", "id": "design", "status": "WORKING"}]))
