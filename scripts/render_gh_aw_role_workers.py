#!/usr/bin/env python3
"""Render bounded specialized gh-aw role-worker sources from trusted registries."""
...
AUTHORING_TEMPLATE = r'''---
name: __NAME__
run-name: "AI-SDLC gh-aw authoring __ROLE__ ${{ inputs.feature_id }}:${{ fromJSON(inputs.task_payload).task.id }}:r${{ inputs.expected_revision }}"
on:
  workflow_dispatch:
    inputs:
      feature_id:
      ...
      - name: Dispatch authoring payload to trusted canonical writer
        ...
        run: |
          ...
          GH_TOKEN="$TRIGGER_TOKEN" gh workflow run ai-sdlc-gh-aw-authoring-result.yml \
            ...
            --field persist=true
---
# AI-SDLC bounded autonomous __ROLE_LABEL__ authoring worker

You are the AI-SDLC __ROLE_LABEL__ authoring worker for stage `__STAGE__`. You produce artifact content only; you are not repository-write or lifecycle authority.
...
3. Author only the bounded `__ARTIFACT_LABEL__` content required by the assigned task. For remediation, address only durable review feedback in the trusted task payload.
...
def render_gate(worker, profile):
...
def render_authoring(worker, profile):
    labels={
        ("product","requirement"):("Product Requirement", "Requirement"),
        ("architect","design"):("Architect Design", "Design"),
        ("orchestrator","plan"):("Orchestrator Plan", "Plan"),
    }
    label,artifact_label=labels[(worker.role,worker.stage)]
    return (AUTHORING_TEMPLATE
        .replace("__NAME__", f"AI-SDLC gh-aw {label} ({worker.profile})")
        .replace("__ROLE__", worker.role)
        .replace("__ROLE_LABEL__", label)
        .replace("__STAGE__", worker.stage)
        .replace("__ARTIFACT_LABEL__", artifact_label)
        .replace("__ENGINE__", engine_block(profile)))


def render(worker, profile):
    key=(worker.role,worker.stage)
    if key in GATE_ROLE_STAGES: return render_gate(worker,profile)
    if key in AUTHORING_ROLE_STAGES: return render_authoring(worker,profile)
    raise ValueError(f"unsupported specialized worker: {worker.role}/{worker.stage}")
...
def main():
    ...
    print("gh-aw specialized role-worker sources are deterministic")
