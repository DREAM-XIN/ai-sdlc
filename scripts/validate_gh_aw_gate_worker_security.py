...
def main():
    ...
    local_qa_source=(ROOT/".github/workflows/ai-sdlc-gh-aw-qa-gemini-v03-local.md").read_text(encoding="utf-8")
    local_qa_lock=(ROOT/".github/workflows/ai-sdlc-gh-aw-qa-gemini-v03-local.lock.yml").read_text(encoding="utf-8")
    ...
    require("safe-outputs:\n  threat-detection:\n    engine:\n      id: copilot\n      harness:\n        max-retries: 0" in local_source,"v0.3 local Reviewer threat detection must retain zero harness retries")
    ...
    require('version: "0.39.1"' in local_qa_source,"v0.3 local QA must pin auth-compatible Gemini CLI 0.39.1")
    require('"engine_versions":{"gemini":"0.39.1"}' in local_qa_lock,"compiled v0.3 local QA must retain Gemini CLI 0.39.1 metadata")
    require("@google/gemini-cli@0.39.1" in local_qa_lock,"compiled v0.3 local QA must install auth-compatible Gemini CLI 0.39.1")
