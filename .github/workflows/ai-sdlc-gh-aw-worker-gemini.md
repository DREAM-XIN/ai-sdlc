...
5. Produce the concrete bounded output required by `feature_context` and, when linked, the Feature Issue acceptance criteria. If they name an exact file or output, use that exact target. Only when no more specific output is defined should you fall back to one small documentation artifact recording the bounded task goal, what was changed, and how the change was verified. Keep it factual and concise.
