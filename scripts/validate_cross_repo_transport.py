...
def main():
    ...
    require("workflow = 'ai-sdlc-persist.yml'" in command, "command bridge does not bind persist to the installed caller")
    ...
    require("--field event_path=\"$EVENT_PATH\"" in command, "trusted persist command does not bind the Event Inbox path")
    require("--field feature_issue=\"$FEATURE_ISSUE\"" in command, "trusted persist command does not bind the source Feature Issue")
    require("--field dry_run=false" in command, "trusted persist command does not request durable persistence")
    require(command.count("--field allow_default_branch=false") >= 2, "trusted bootstrap/persist commands lost default-branch denial")
    ...
    require("/ai-sdlc persist " in command and "event=(state/events/" in command, "persist command syntax is not exact/durable-state bounded")
    require("state/bootstrap/" in command and "state/features/" in command and "state/events/" in command, "command parser does not constrain durable state paths")
    ...
    require("Event: \\`$EVENT_PATH\\`" in command, "persist receipt does not record the Event path")
