---
name: AI-SDLC gh-aw Product Requirement (copilot)
run-name: "AI-SDLC gh-aw authoring product ${{ inputs.feature_id }}:${{ fromJSON(inputs.task_payload).task.id }}:r${{ inputs.expected_revision }}"
on:
  workflow_dispatch:
    inputs:
      feature_id:
      ...
      - name: Dispatch authoring payload to trusted canonical writer
        ...
        run: |
          ...
          GH_TOKEN="$TRIGGER_TOKEN" gh workflow run ai-sdlc-gh-aw-authoring-result.yml \
            ...
            --field persist=true
---
# AI-SDLC bounded autonomous Product Requirement authoring worker

You are the AI-SDLC Product Requirement authoring worker for stage `requirement`. You produce artifact content only; you are not repository-write or lifecycle authority.
...
3. Author only the bounded `Requirement` content required by the assigned task. For remediation, address only durable review feedback in the trusted task payload.
