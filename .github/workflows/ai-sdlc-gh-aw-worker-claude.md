...
3. Before editing, create and switch to a unique local work branch for this run, for example `gh-aw/${{ inputs.feature_id }}-${{ github.run_id }}`. Confirm `git branch --show-current` is this new work branch before making changes. `${{ inputs.target_ref }}` is the reserved Feature branch and **PR base only**; never use `${{ inputs.target_ref }}` as the local work branch or as `create_pull_request.branch`.
...
6. Review the diff before finishing. If anything outside `docs/gh-aw-dogfood/` changed, revert it. Commit the bounded change on the unique local work branch. Do not push it yourself.
7. **Submission phase 1 is mandatory:** call the `create_pull_request` safe-output tool with the bounded diff. Set its `branch` argument to exactly the current local work branch reported by `git branch --show-current`. Do not set or override the PR base; the trusted Safe Output configuration already fixes the base to `${{ inputs.target_ref }}`. The head branch and `${{ inputs.target_ref }}` must be different. Do not claim completion, emit `noop`, or call `report_result` before requesting this PR.
8. **Submission phase 2 is mandatory and ordered:** only after the `create_pull_request` request has been emitted, call the `report_result` safe-output tool with a non-empty concise completion summary. The deterministic post-safe-output job independently verifies that exactly one matching open Draft PR exists before it can dispatch a COMPLETED Worker Result.

If `create_pull_request` rejects the branch/base relationship, stop rather than retrying with `${{ inputs.target_ref }}` as the head branch. If you cannot request the Draft PR, do not report completion. Do not edit `state/features/**` or `state/events/**`. Do not pass or waive any Gate. Do not merge the PR. Independent AI-SDLC review remains a later stage.
