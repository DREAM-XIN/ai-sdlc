...
or retry exhaustion remains fail-closed.  The binding is process-local and is
never serialized.
"""
...
class CausalSummarySettledAttestedGitHubOperatorStoreRulesetProvisioner(
    ...
    def protection_verifier(self):
        ...
        # Exact summary replay fences are intentionally scoped to this one
        # process-local verifier.  Nothing here is persisted as authority.
        ...
        def get(url: str, headers: dict[str, str]):
            ...
            if exact_key is None:
                # Most malformed/older/newer shapes are already rejected by the
                # inherited verifier.  One subtle case needs an explicit
                # process-local guard: the inherited equality check would treat
                # two missing updated_at values as equal (None == None).  Record
                # that exact-version malformed observation so this proof cannot
                # accidentally close even if both raw reads are identically bad.
            ...
            if expected is not None:
                ...
                # The same exact version is visible but metadata came from a
                # different replica.  Do not move the replay fence.  Wait only
                # for GitHub itself to return the originally bound real pair.
            ...
            # consecutive real observations of the same exact summary before
            ...
            # bound.  Return the last real response; never manufacture the
            # candidate pair or mutate its metadata.
            return last_status, last_value
