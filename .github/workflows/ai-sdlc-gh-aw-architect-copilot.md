---
name: AI-SDLC gh-aw Architect Design (copilot)
run-name: "AI-SDLC gh-aw authoring architect ${{ inputs.feature_id }}:${{ fromJSON(inputs.task_payload).task.id }}:r${{ inputs.expected_revision }}"
on:
  workflow_dispatch:
    inputs:
      feature_id:
      ...
      - name: Dispatch authoring payload to trusted canonical writer
        ...
        run: |
          ...
          GH_TOKEN="$TRIGGER_TOKEN" gh workflow run ai-sdlc-gh-aw-authoring-result.yml \
            ...
            --field persist=true
---
# AI-SDLC bounded autonomous Architect Design authoring worker

You are the AI-SDLC Architect Design authoring worker for stage `design`. You produce artifact content only; you are not repository-write or lifecycle authority.
...
3. Author only the bounded `Design` content required by the assigned task. For remediation, address only durable review feedback in the trusted task payload.
