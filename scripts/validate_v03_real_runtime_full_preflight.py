...
def validate_positive_preflight_is_fixed_and_zero_effect():
    require(
        subject.DEVELOPER_WORKFLOW == "ai-sdlc-gh-aw-worker-codex.lock.yml",
    ...
    for workflow in (
        ...
        path = subject.WORKFLOW_DIR / workflow
        require(path.is_file() and not path.is_symlink(), f"expected production Worker is not installed: {workflow}")
...
def validate_missing_configured_worker_fails_before_composition():
    original = subject.DEVELOPER_WORKFLOW
    subject.DEVELOPER_WORKFLOW = "definitely-not-installed-v03-worker.lock.yml"
    try:
        _expect_rejected()
    finally:
        subject.DEVELOPER_WORKFLOW = original
...
def main():
    ...
    validate_missing_configured_worker_fails_before_composition()
    validate_fixture_candidate_fails_closed_before_launch()
    print("PASS: v0.3 full-runtime preflight binds exact installed main/policy/reviewer/fixture/composition without launch")
