#!/usr/bin/env python3
"""Regression for #336/#338 trusted ruleset current-detail write binding."""
...
from operator_store_github_ruleset_current_detail_bound import (
    ...
    _current_detail_binds_fresh_marker,
...
from operator_store_github_ruleset_stabilized import NormalizedAttestedGitHubRulesetProtectionVerifier
...
STALE_UPDATED_AT = "2026-08-24T14:59:59Z"
VERSION_ID = 500
...
def _current(payload: dict, *, source=REPOSITORY, updated_at=UPDATED_AT, rules=None) -> dict:
    value = _state(payload, source=source, rules=_omission_rules() if rules is None else rules)
...
class BoundHistoryApi:
    ...
    def request(self, method: str, url: str, headers: dict[str, str], body=None):
        ...
        if method == "GET" and path == f"{history_path}/{VERSION_ID}":
            return 200, {"version_id": VERSION_ID, "updated_at": UPDATED_AT, "state": copy.deepcopy(self.history_state)}
        return 404, {}
...
def _attempt_write_then_wait(api: BoundHistoryApi, payload: dict, *, minimum_version_id=None):
    ...
    return provisioner._wait_for_exact_history_state(
        REPOSITORY, ruleset_id, payload, minimum_version_id=minimum_version_id
    )
...
def validate_fresh_marker_can_bind_without_timestamp_equality():
    payload = _payload(marker=True)
    current = _current(payload, updated_at=STALE_UPDATED_AT)
    api = BoundHistoryApi(history_state=_opaque_history(payload), current_detail=current)
    version_id, state = _attempt_write_then_wait(api, payload)
    require(version_id == VERSION_ID, "fresh marker returned wrong causal version")
    require(state == _state(payload), "fresh marker did not canonicalize exact strict state")
    require(api.put_calls == 1, "fresh marker fallback was not bound to this process write")
    require(api.get_detail_calls == 1, "fresh marker did not re-read exact current detail")
    require(
        _current_detail_binds_fresh_marker(
            current, repository=REPOSITORY, ruleset_id=RULESET_ID, payload=payload
        ),
        "fresh marker helper rejected exact marker identity",
    )


def validate_fresh_marker_fallback_requires_same_process_write():
    payload = _payload(marker=True)
    api = BoundHistoryApi(
        history_state=_opaque_history(payload),
        current_detail=_current(payload, updated_at=STALE_UPDATED_AT),
    )
    provisioner = _provisioner(api)
    try:
        provisioner._wait_for_exact_history_state(REPOSITORY, RULESET_ID, payload)
    except RulesetProvisioningError:
        pass
    else:
        raise AssertionError("fresh marker gained authority without same-process pending write")
    require(api.put_calls == 0, "same-process negative case unexpectedly wrote a ruleset")
    require(api.get_detail_calls == 0, "unbound marker should not consult current-detail authority")


def validate_canonical_timestamp_binding_fails_closed():
    payload = _payload(marker=False)
    api = BoundHistoryApi(
        history_state=_opaque_history(payload),
        current_detail=_current(payload, updated_at=STALE_UPDATED_AT),
    ...
    require("after 1 attempts" in text, "canonical stale timestamp gained retry authority")
    require(api.get_detail_calls == 1, "canonical stale timestamp did not exercise exact binding")


def validate_current_detail_identity_fails_closed():
    payload = _payload(marker=True)
    cases: list[tuple[str, dict]] = []
    cases.append(("owner-only-current-source", _current(payload, source=REPOSITORY.split("/", 1)[0], updated_at=STALE_UPDATED_AT)))
    cases.append(("opaque-current-source", _current(payload, source=OPAQUE_SOURCE, updated_at=STALE_UPDATED_AT)))

    extra_rule = _current(payload, updated_at=STALE_UPDATED_AT)
    extra_rule["rules"].append({"type": "deletion"})
    cases.append(("extra-current-rule", extra_rule))

    extra_key = _current(payload, updated_at=STALE_UPDATED_AT)
    ...
    permissive = _current(
        ...
        rules=[{"type": "creation"}, {"type": "update", "parameters": {"update_allows_fetch_and_merge": True}}],
    )
    cases.append(("permissive-current-rule", permissive))

    drift = _current(payload, updated_at=STALE_UPDATED_AT)
    drift["enforcement"] = "disabled"
    cases.append(("current-identity-drift", drift))

    wrong_marker = _current(payload, updated_at=STALE_UPDATED_AT)
    ...
    for label, current in cases:
        ...
        require(api.get_detail_calls == 1, f"{label} did not test exact marker/current boundary")


def validate_marker_name_shape_is_exact():
    require(_is_fresh_marker_name(_payload(marker=True)["name"]), "valid marker name was rejected")
    for value in (
        "AI-SDLC Operator Store writer",
        "AI-SDLC Operator Store writer [attest:]",
        "AI-SDLC Operator Store writer [attest:AABBCCDD]",
        "AI-SDLC Operator Store writer [attest:not-hex]",
        "AI-SDLC Operator Store writer [attest:aabbccdd",
        None,
    ):
        require(not _is_fresh_marker_name(value), f"malformed marker shape gained authority: {value!r}")

    malformed = _payload(marker=True)
    malformed["name"] = "AI-SDLC Operator Store writer [attest:AABBCCDD]"
    api = BoundHistoryApi(
        history_state=_opaque_history(malformed),
        current_detail=_current(malformed, updated_at=STALE_UPDATED_AT),
    )
    _expect_failure(api, malformed)
...
def validate_history_side_remains_exact_and_closed():
    ...
    cases: list[tuple[str, dict]] = []
    for label, source in (("owner-only", owner), ("repo-only", repo), ("non-string", {"not": "a source"})):
        cases.append((label, _state(payload, source=source, rules=_omission_rules())))

    permissive_rules = _opaque_history(payload)
    permissive_rules["rules"] = [{"type": "creation"}, {"type": "update", "parameters": {"update_allows_fetch_and_merge": True}}]
    ...
    for label, history_state in cases:
        api = BoundHistoryApi(history_state=history_state, current_detail=_current(payload, updated_at=STALE_UPDATED_AT))
        text = _expect_failure(api, payload)
        require("after 1 attempts" in text, f"{label} gained cross-surface authority")
        require(api.get_detail_calls == 0, f"{label} should fail history eligibility before consulting current detail")


def validate_permissive_submitted_payload_never_binds():
    payload = _payload()
    payload["rules"] = [{"type": "creation"}, {"type": "update", "parameters": {"update_allows_fetch_and_merge": True}}]
    history = _state(payload, source=OPAQUE_SOURCE, rules=_omission_rules())
    current = _state(payload, rules=_omission_rules())
    current["updated_at"] = STALE_UPDATED_AT
...
def validate_stale_version_never_binds():
    payload = _payload()
    api = BoundHistoryApi(history_state=_opaque_history(payload), current_detail=_current(payload, updated_at=STALE_UPDATED_AT))
...
def validate_helper_requires_exact_write_response_binding():
    ...
    require(
        ...
        is None,
        "post-attestation helper accepted mismatched write-response timestamp",
    )
...
def validate_only_bound_trusted_verifier_can_revalidate_opaque_source():
    ...
    legacy = NormalizedAttestedGitHubRulesetProtectionVerifier(
        token=SECRET, operator_app_id=APP_ID, http_get=api.get, write_attestations=attestations
    )
    require(
    ...
    require(
        ...
        "post-attestation verifier accepted changed current-detail write binding",
    )


def main():
    validate_exact_current_detail_can_bind_opaque_history_source()
    validate_fresh_marker_can_bind_without_timestamp_equality()
    validate_fresh_marker_fallback_requires_same_process_write()
    validate_canonical_timestamp_binding_fails_closed()
    validate_current_detail_identity_fails_closed()
    validate_marker_name_shape_is_exact()
